...
- `blocked_by: [parent_slug]` — 子任务依赖 parent
- `context_pointers` 中包含 `parent: task-N` 引用
- `for_agent: true`（默认）
...
Parent 的 `for_agent` 改为 `false`——它的完成条件是所有子任务全部 verify 过。

**Completion criterion:** 所有子任务已落库；parent acceptance_criteria 已更新；用户看到完整结构树。
...
- **每个子 Task 独立可执行。** acceptance_criteria 不能隐含"等其他 task 完成"——需要顺序用 `blocked_by` 声明。
- **子 Task 不能循环依赖。** `blocked_by` 只能指向 parent 或同层前置 task，不允许环路。
...
```
Spec: task-N (for_agent: false)
├── task-N1: <title> [for_agent: true]
├── task-N2: <title> [for_agent: true]
└── task-N3: <title> [for_agent: true]
...
| 子任务忘了加 blocked_by 指向 parent              | 每次 create 子任务时 parent_slug 必须出现在 blocked_by 里                            |
