...
**只执行可执行 task（`for_agent=true`）。** Parent task（`for_agent=false`）是追踪节点，不接受直接执行——遇到时拒绝并列出其可执行子任务。
...
- **`for_agent: false`** → 这是 parent task（追踪节点）。**拒绝执行**。用 `list_children(slug)` 获取其子任务集，列出子 task 的 slug + title，建议用户执行其中状态非 `已完成` 的子任务。本技能不执行 parent。
- **`for_agent: true` + `blocked_by` 非空** → 这是子任务。检查 blocker 任务状态：
  - blocker 不是 `已完成` → 先执行 blocker（告知用户并建议 slug）
  - blocker 是 `已完成` / 已 verify → 正常继续
- **`for_agent: true` + `blocked_by` 为空** → 原子任务，正常继续。

**Completion criterion:** 手上有任务的完整 spec；MCP 连通性已确认；parent 场景已获取完整子任务列表。

### 步骤 2：读上下文

按 `context_pointers` 列出的路径读代码 / 文档。**不靠记忆，靠读文件**。context_pointers 没覆盖但明显相关的文件，补读。

如果当前任务是子任务（`blocked_by` 非空），还要 **读 parent spec 获取整体方案上下文**——parent 的 `context_pointers` 和 `detail` 包含步骤 2a 产出的方案描述，是理解"为什么这么做"的关键。
...
如果当前任务是子任务（`blocked_by` 非空），**检查同组兄弟任务是否全部 `已完成`**：

1. 通过 `list_children(parent_slug)` 获取全部子任务
...
2. **上下级关系**（如适用）：
   - 如果是子任务 → 展示 parent slug + 所有子任务完成进度（`3/5 已完成`）
   - 如果 parent 已被自动完成 → 告知用户"parent task-N 已自动标完成"
3. **后续动作**：
   - 有未完成的兄弟子任务 → 提示"下一个建议执行：`devtask:devtask-doit task-N+1`"
   - parent 已全部完成 → 提示"整个 spec 已完成"
   - 有其他任务被当前任务阻塞 → 列出下一个可执行的 task
