...
argument-hint: [Which task do you want to deal with? <task-slug>]
---
...
**只执行可执行 task（`for_agent=true`、`kind=subtask`）。** Parent spec（`for_agent=true`、`kind=spec`）是追踪节点，不接受直接执行——遇到时拒绝并列出其可执行子任务。
...
没 slug → `get_frontier_tasks(limit=1)`，取第一个。返回空则告知用户"frontier 为空，没有可执行的任务"并结束。frontier 返回的数组里每个任务**不**含嵌套 parent，如需 parent 上下文再单独调用 `get_dev_task_by_slug(slug, with_parent=True)`。

拿到任务后做以下判断：

- **`kind == "spec"`** → 这是 parent spec（追踪节点）。**拒绝执行**。用 `list_children(slug)` 获取其子任务集，列出子 task 的 slug + title，建议用户执行其中状态非 `已完成` 的子任务。本技能不执行 parent。
...
**优化：** 在步骤 1 首次获取子任务时直接调用 `get_dev_task_by_slug(slug, with_parent=True)`，后端会在同一次响应里附带 `parent` 字段（父 spec 完整数据），省去步骤 2 的一次额外往返。只有当步骤 1 未使用 `with_parent`（例如 frontier 领取返回的数组里没有嵌套 parent）才需单独再查。
...
1. 通过 `list_children(parent_slug)` 获取全部子任务（后端 parent_slug 索引查询）
2. 如果所有子任务都已 `已完成` → 用 `update_dev_task(parent_slug, status="已完成")` 自动完成 parent
3. 如果还有未完成的子任务 → 在交付中列出，提示用户可以继续执行下一个

**注：** 步骤 1 已经通过 `with_parent=True` 获取了 parent 数据，这里无需再次调用 `get_dev_task_by_slug(parent_slug)` 来确认 parent 存在——直接使用步骤 1 拿到的 `parent` 即可。如果步骤 1 未使用 `with_parent`，则需要单独调用 `get_dev_task_by_slug(parent_slug)` 确认。
