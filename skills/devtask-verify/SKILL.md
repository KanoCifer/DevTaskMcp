...
**Parent task 递归验证。** 如果被验证的是 parent task（`for_agent=false`、有子任务），会自动递归验证所有子任务，并将 parent 的验收条件（"所有子任务 verify 通过"）作为聚合结论。
...
- **`for_agent: false` + `acceptance_criteria` 含子任务引用**（如 `- [ ] task-N1:`）→ **Parent task**：需要递归验证所有子任务
- **`for_agent: true`** → **子任务 / 原子任务**：标准逐条验证

如果是 parent task，用 `list_children(slug)` 一次性获取全部子任务（工具内部处理分页，返回完整 task 对象无需再次 get_dev_task_by_slug）。

**Completion criterion:** 手上有任务的完整 spec + 层级判定结果；parent 已获取完整子任务列表。
...
- **子任务完成类** — `- [ ] task-N1: ... verify 通过` → 解析出子 task slug，后续通过子任务验证结果判定
...
- **子任务完成类（parent 特有）：** 步骤 1 已通过 `list_children` 拿到子任务的完整 spec（含 acceptance_criteria、context_pointers），直接逐条验证每条子任务的验收条件（无需再次调用 get_dev_task_by_slug）。子任务全部 ✅ → 本条 ✅；任一 ❌ → 本条 ❌。证据引用子报告的结论。
...
- **Parent 的子任务找不到：** 如果 `list_dev_tasks` 无法找到 `blocked_by` 包含 parent slug 的子任务，告知用户子任务可能未正确关联，建议检查 `blocked_by` 字段。
